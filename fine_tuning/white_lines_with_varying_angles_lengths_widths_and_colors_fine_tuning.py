...
output_dir = r"Path_to_Model\white_lines_with_varying_angles_lengths_widths_colors_fine_tuned_model"
os.makedirs(output_dir, exist_ok=True)
print(f"Output directory set to: {output_dir}")
excel_path = os.path.join(r"Path_to_Labels\white_lines_with_varying_angles_lengths_widths_colors", "line_properties.xlsx")
...
class LineDataset(Dataset):
    def __init__(self, image_dir, excel_path, max_samples=50000, transform=None):
        ...
        self.df['line_length'] = self.df['length'] / 224.0  # Normalize length to [0,1]
        self.df['line_width'] = self.df['width'] / 10.0  # Normalize width assuming max width of 10 pixels
        
        # Convert color to numerical values
        self.df['color_rgb'] = self.df['color_rgb'].apply(lambda x: np.array(eval(x))/255.0)  # Normalize RGB to [0,1]
    ...
    def __getitem__(self, idx):
        ...
        target = {
            ...
            'color_rgb': torch.tensor(self.df.iloc[idx]['color_rgb'], dtype=torch.float32)
        }
        return image, target
...
class ViTRegression(torch.nn.Module):
    def __init__(self, pretrained_model='google/vit-base-patch16-224'):
        ...
        self.color_head = torch.nn.Sequential(
            torch.nn.Linear(hidden_size, hidden_size//2),
            torch.nn.GELU(),
            torch.nn.Linear(hidden_size//2, 3),
            torch.nn.Sigmoid()  # RGB values between 0 and 1
        )
        
        # Initialize with smaller weights
        for layer in [self.shared_head, self.angle_head, self.coord_head, 
                     self.noise_head, self.length_head, self.width_head, self.color_head]:
...
def weighted_mse_loss(preds, targets):
    ...
    other_loss = (torch.nn.functional.huber_loss(preds['noise_level'], targets['noise_level'], reduction='mean', delta=0.1) +
                 ...
                 torch.nn.functional.huber_loss(preds['line_width'], targets['line_width'], reduction='mean', delta=0.1) +
                 torch.nn.functional.huber_loss(preds['color_rgb'], targets['color_rgb'], reduction='mean', delta=0.1)) * 0.5
...
def save_figures(output_dir, train_losses, val_losses, inference_times, 
    ...
    features = ['angle', 'start_x', 'start_y', 'end_x', 'end_y', 'noise_level', 'line_length', 'line_width', 'color_r', 'color_g', 'color_b']
...
def calculate_correlations(preds, targets):
    ...
    try:
        for i, channel in enumerate(['r', 'g', 'b']):
            correlations[f'color_{channel}'] = np.corrcoef(preds_np['color_rgb'][:, i], targets_np['color_rgb'][:, i])[0, 1]
    except:
        for channel in ['r', 'g', 'b']:
            correlations[f'color_{channel}'] = 0
...
def calculate_errors(preds, targets):
    ...
    # Color channels
    for i, channel in enumerate(['r', 'g', 'b']):
        errors[f'color_{channel}'] = preds_np['color_rgb'][:, i] - targets_np['color_rgb'][:, i]
...
def main():
    ...
    full_dataset = LineDataset(
        r"PAth_to_Images\white_lines_with_varying_angles_lengths_widths_colors",
        excel_path
    )
    ...
    for epoch in range(100):
        ...
        model.train()
        train_loss = 0
        epoch_train_preds = {k: [] for k in ['angle', 'start_point', 'end_point', 'noise_level', 'line_length', 'line_width', 'color_rgb']}
        epoch_train_targets = {k: [] for k in ['angle', 'start_point', 'end_point', 'noise_level', 'line_length', 'line_width', 'color_rgb']}
        ...
        model.eval()
        val_loss = 0
        epoch_val_preds = {k: [] for k in ['angle', 'start_point', 'end_point', 'noise_level', 'line_length', 'line_width', 'color_rgb']}
        epoch_val_targets = {k: [] for k in ['angle', 'start_point', 'end_point', 'noise_level', 'line_length', 'line_width', 'color_rgb']}
        ...
        features_to_print = ['angle', 'start_x', 'start_y', 'end_x', 'end_y', 'noise_level', 'line_length', 'line_width', 'color_r', 'color_g', 'color_b']
