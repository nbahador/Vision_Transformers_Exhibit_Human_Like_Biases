...
output_dir = r"Path_to_Model\white_lines_varying_angles_fine_tuned_model"
os.makedirs(output_dir, exist_ok=True)
print(f"Output directory set to: {output_dir}")
excel_path = os.path.join(r"Path_to_Labels\white_lines_varying_angles", "line_labels.xlsx")
...
class LineDataset(Dataset):
    def __init__(self, image_dir, excel_path, max_samples=50000, transform=None):
        ...
        self.df['angle'] = (self.df['angle'] % 360) / 180.0 - 1  # Normalize angles to [-1,1] range
...
class ViTRegression(torch.nn.Module):
    def __init__(self, pretrained_model='google/vit-base-patch16-224'):
        ...
        for layer in [self.shared_head, self.angle_head, self.coord_head, self.noise_head, self.length_head]:
...
def weighted_mse_loss(preds, targets):
    ...
    other_loss = (torch.nn.functional.huber_loss(preds['noise_level'], targets['noise_level'], reduction='mean', delta=0.1) +
                 torch.nn.functional.huber_loss(preds['line_length'], targets['line_length'], reduction='mean', delta=0.1)) * 0.5
...
def save_figures(output_dir, train_losses, val_losses, inference_times, 
    ...
    features = ['angle', 'start_x', 'start_y', 'end_x', 'end_y', 'noise_level', 'line_length']
...
def main():
    ...
    full_dataset = LineDataset(r"Path_to_Images\white_lines_varying_angles", excel_path)
    ...
    for epoch in range(100):
        ...
        model.train()
        train_loss = 0
        epoch_train_preds = {k: [] for k in ['angle', 'start_point', 'end_point', 'noise_level', 'line_length']}
        epoch_train_targets = {k: [] for k in ['angle', 'start_point', 'end_point', 'noise_level', 'line_length']}
        ...
        model.eval()
        val_loss = 0
        epoch_val_preds = {k: [] for k in ['angle', 'start_point', 'end_point', 'noise_level', 'line_length']}
        epoch_val_targets = {k: [] for k in ['angle', 'start_point', 'end_point', 'noise_level', 'line_length']}
        ...
        features_to_print = ['angle', 'start_x', 'start_y', 'end_x', 'end_y', 'noise_level', 'line_length']
